# 👋 Hi, I'm Yanting (Iris) 

### 📊 Business Analytics Graduate Student @ University of Dayton
> **10 Years of Manufacturing Operations Experience | Data Automation Specialist**

I bridge the gap between complex manufacturing operations and data-driven decision-making. Currently pursuing a Master's in Business Analytics to leverage my extensive background in cost accounting and operational analysis through modern data technology.

---

### 🛠️ Technical Toolkit
* **Data Analysis:** Python (Pandas, NumPy), SQL, Excel (VBA)
* **Visualization:** Streamlit, Tableau, Matplotlib
* **Tools:** Git, GitHub, Jupyter Notebook, SAP/ERP Systems

---

### 🚀 Featured Projects
#### [📊 Production Data Automation Dashboard](https://your-streamlit-link-here.streamlit.app/)
* Developed a real-time web application to automate the consolidation of multi-workshop CSV reports.
* Reduced data processing time by **90%** and provided instant KPI insights (Output & Waste Rate).
* *Tech: Python, Streamlit, Pandas*

#### [📉 Manufacturing Data Integration Macro](https://github.com/Yanting1992/excel-automation)
* Automated complex Excel workflows for cost accounting, ensuring 100% data accuracy.
* *Tech: VBA, Excel*

---

### 📈 Professional Journey
* **Graduate Student**, Business Analytics | *University of Dayton* (Expected Dec 2026)
* **Operations Analyst / Cost Accountant** | *Fuyao Glass* (10 Years)
    * Focused on operational efficiency, cost control, and financial reporting.

---

### 📫 Connect with Me
* **LinkedIn:** [Your LinkedIn Profile Link]
* **Location:** Dayton, Ohio (Open to roles in the Ohio region)
* **Goal:** Seeking Data Analyst / Business Analyst roles starting Winter 2026.

![Your GitHub Stats](https://github-readme-stats.vercel.app/api?username=Yanting1992&show_icons=true&theme=radical)
